    ...
    v-col(col='12')
      smart-table(
        ...
        template(#value--change="{ item }")
          router-link(:to='`/expert-recovery/${item.id}`')
            v-btn(
              @click="showItem(item)"
            ) Изменить
...
export default {
  ...
  methods: {
    async adapter({ _update, ...options }) {
      ...
      const response = adminService.getExpertUsers(params);
      return response;
    },
    SmartTableRunSearch() {
    ...
    // showItem(item) {
    //   this.$router.push(`/expert-recovery/:${item.id}`);
    // },
    submitDeleteUserProfile(id) {
      if (this.isDeleteAllowed !== false) {
        this.$rHandler(adminService.deleteExpertProfile(id),
        ...
        this.$toast.error(`Вы не подтвердили удаление эксперта с id: ${id}`);
      }
    },

...
v-container
  admin-user-change-confirm(
    ...
    changingRole="organization"
    @confirmChanges="checkConfirmation"
    @closeDialog="updateConfirmDialogStatus"
  )
  h1.text-h4.admin__changes__header Изменение организации с id: {{ this.$route.params.id }}
  v-form(
    ref="form"
    v-model="valid"
    lazy-validation
    ...
    v-row
      v-col(
        ...
        ev-text-field(
          ...
          template(#tooltip)
              span Поле ИНН не может быть пустым
    v-row
      v-col(
        cols="6"
      )
    ...
    v-row
      v-col(
        ...
        ev-text-field(
          ...
          v-model="updatedUserData.short_name"
          ...
          template(#tooltip)
            span Поле краткого наименования не может быть пустым
    v-row
      v-col(
        cols="6"
        )
        ev-text-field(
          ...
          v-model="updatedUserData.full_name"
          :rules="[$rules.required]"
          outlined
          dense
          hide-details
          ...
          template(#tooltip)
            span Поле полного наименования не может быть пустым
    v-row
      v-col(
        cols="6"
      )
        ev-text-field(
          label="ФИО директора"
          v-model="updatedUserData.contact_full_name"
          :rules="[$rules.required]"
          outlined
          dense
          hide-details
          ...
          template(#tooltip)
            span Поле ФИО директора не может быть пустым
    v-row
      v-col(
        cols="6"
      )
        ev-text-field(
          ...
          v-model="updatedUserData.contact_phone"
          :rules="[$rules.required]"
          outlined
          dense
          hide-details
          ...
          template(#tooltip)
            span Поле телефона ответственного лица не может быть пустым
    v-row
      v-col(
        cols="6"
      )
        ev-text-field(
          ...
          v-model="updatedUserData.fact_address"
          :rules="[$rules.required]"
          outlined
          dense
          hide-details
          ...
          template(#tooltip)
            span Поле фактического адреса не может быть пустым
    v-row
      v-col(
        cols="6"
      )
        ev-text-field.mb-4(
          ...
          v-model="updatedUserData.legal_address"
          :rules="[$rules.required]"
          outlined
          dense
          hide-details
          ...
          template(#tooltip)
            span Поле юридического адреса не может быть пустым
...
export default {
  name: 'AdminOrganizationChangePage',
  components: {
    DirectoryMunInput,
  ...
  methods: {
    loadSelectedUser() {
      ...
      this.$rHandler(adminService.getOrganizationUsers(params),
        (response) => {
    ...
    submitChangeUserData() {
      if (this.$refs.form.validate()) {
        if (this.isChangeAllowed !== false) {
          ...
          this.$rHandler(adminService.putOrganizationData(this.updatedUserData.id, params),
            () => { this.$toast.success('Изменения в полях организации успешно приняты!'); });

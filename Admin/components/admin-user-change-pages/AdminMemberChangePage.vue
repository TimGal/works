...
v-container
  ...
  h1.text-h4.admin__changes__header Изменение пользователя с id: {{ this.$route.params.id }}
  v-form(
    ref="form"
    v-model="valid"
    lazy-validation
    ...
    v-row
      v-col(
        ...
        ev-datetime-field.mb-4(
          v-model="updatedUserData.birthday_at"
          :rules="[$rules.required]"
          label="Дата рождения"
          only-date
          toplabel
          ...
          template(#tooltip)
            span Поле даты рождения не может быть пустым
...
export default {
  ...
  methods: {
    loadSelectedUser() {
      ...
      this.$rHandler(adminService.getMemberUsers(params),
        (response) => {
    ...
    submitChangeUserData() {
      if (this.$refs.form.validate()) {
        if (this.isChangeAllowed !== false) {
          ...
          this.$rHandler(adminService.putMemberData(this.updatedUserData.id, params),
            () => {
              this.loadSelectedUser();
              this.$toast.success('Изменения в полях пользователя успешно приняты!');
            });

...
v-container
  admin-user-change-confirm(
    ...
    changingRole="trainer"
    @confirmChanges="checkConfirmation"
    @closeDialog="updateConfirmDialogStatus"
  )
  h1.text-h4.admin__changes__header Изменение тренера с id: {{ this.$route.params.id }}
  v-form(
    ref="form"
    v-model="valid"
    lazy-validation
    ...
    v-row
      v-col(
        ...
        ev-text-field(
          ...
          template(#tooltip)
              span Поле фамилия не может быть пустым
    v-row
      v-col(
        cols="6"
        )
        ev-text-field.mb-4(
          ...
          template(#tooltip)
              span Поле отчество не может быть пустым
...
export default {
  ...
  methods: {
    loadSelectedUser() {
      ...
      this.$rHandler(adminService.getTrainerUsers(params),
        (response) => {
    ...
    submitChangeUserData() {
      if (this.$refs.form.validate()) {
        if (this.isChangeAllowed !== false) {
          ...
          this.$rHandler(adminService.putTrainerData(this.updatedUserData.id, params),
